...
func CreateStripedPatternScene(width int, height int) *canvas.Canvas {
	...
	yellowHorizontalStripes := g.CreateStripePattern(math.CreateColor(1.0, 1.0, 1.0), math.CreateColor(1.0, 1.0, 0.0))
	yellowHorizontalStripes.SetTransform(math.Rotation_Z(gomath.Pi / 2.0))

	tealPattern := g.CreateStripePattern(math.CreateColor(0.0, 0.0, 0.0), math.CreateColor(0.0, 1.0, 1.0))
